<template>
	<div
		...
		:class="{ active: isDragover, hasImage: !!imageSrc }"
		@drag="preventEvent"
	...
	>
		<label :for="id" @drop="onDrop" @dragover.prevent="onDragover" @dragleave.prevent="onDragleave">
			<template v-if="isDragover">Drop image here.</template>
			<template v-if="!imageSrc && !isDragover"> {{ placeholder }} </template>
			<div class="image-preview" v-if="imageSrc && !isDragover">
...
export default {
	props: {
		...
		placeholder: { type: String, default: "Drag and drop image here or click to select image to upload." },
		...
		value: { type: [String, File], default: "" },

		beforeUpload: { type: Function, default: () => {} },

		minHeight: { type: [String, Number], default: "180px" },

		maxHeight: { type: [String, Number], default: "230px" }
	},

	data() {
		return {
			isDragover: false,
			id: "default-drag-fileinput_"
		};
	},
	...
	computed: {
		imageSrc() {
			let image = this.image || this.value;
